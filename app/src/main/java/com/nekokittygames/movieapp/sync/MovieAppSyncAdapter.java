...
public class MovieAppSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private final MovieDbService mMovieService;

    public MovieAppSyncAdapter(Context context, @SuppressWarnings("SameParameterValue") boolean autoInitialize) {
        super(context, autoInitialize);
        Retrofit mRetrofit = new Retrofit.Builder().baseUrl("http://api.themoviedb.org/").addConverterFactory(GsonConverterFactory.create()).build();
    ...
    private static void configurePeriodicSync(Context context, int syncInterval, int flexTime) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            SyncRequest request = new SyncRequest.Builder().
                    syncPeriodic(MovieAppSyncAdapter.SYNC_INTERVAL, flexTime).
                    setSyncAdapter(account, authority).
                    setExtras(new Bundle()).build();
            ContentResolver.requestSync(request);
        } else {
            ContentResolver.addPeriodicSync(account,
                    authority, new Bundle(), MovieAppSyncAdapter.SYNC_INTERVAL);
        }
    }
